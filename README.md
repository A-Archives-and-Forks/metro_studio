...
- 车辆 HUD:
  ...
  - 关键 DOM/SVG 节点已补充语义化 class（`hud-header__*` / `hud-track__*` / `hud-station__*`），便于在 DevTools 中快速定位元素并做样式覆写。
  - HUD 站序自动按方向展开，渲染蓝色主线、空心站点、段间双箭头与中英站名。
  - 顶部横条按济南地铁实车 HUD 风格重构：左侧主色线路区（Logo + 线路牌）、中部白色“到达站 / Arrived + 到站名”面板、右侧主色方向区（`起始站 >>> 到达站`）。
