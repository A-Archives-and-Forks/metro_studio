...
- AI 点站模式（点击后抓取周边 300m OSM 道路/地域/公共设施/建筑，并调用本机 Ollama 给出 5 个命名候选，鼠标处弹出菜单选择）
...
- 启动 Ollama 服务（默认 `http://127.0.0.1:11434`）
- 已拉取模型 `kamekichi128/qwen3-4b-instruct-2507:latest`
...
- `src/lib/ai/`：本机 Ollama 命名候选生成（结构化 JSON 输出校验与回退）。
...
- 编辑交互:
  ...
  - 新增 AI 点站模式：空白点击后先落站，再抓取周边 `300m` OSM 语义要素（道路/地域/公共设施/建筑），调用本机 Ollama（`kamekichi128/qwen3-4b-instruct-2507:latest`）返回 `5` 个候选站名，并在鼠标处弹出可点击菜单应用中英文站名。
  ...
  - 地图右键菜单按点击对象动态显示：空白处（加站/AI加站/清空选择）、站点（重命名/删除）、线段（加锚点/删线段/清空锚点）、锚点（删锚点/加锚点/清空所属线段锚点）；并在鼠标位置附近按实际菜单尺寸自动贴边。
